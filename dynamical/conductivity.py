...
from residence_time import _set_axis


class Conductivity:
    def __init__(self, filename, num_mols, num_atoms_per_mol, mass=None,
                 timestep=1, working_dir=None):
    ...
    def define_mol_cols(self, dump):
        ...
        if not self.mass:
            ...
            df = pd.DataFrame(dump.data[['id', 'type', 'vx', 'vy', 'vz', 'q']],
        ...
        mol_df = df.groupby(['mol_type', 'mol_id']). \
            apply(
            lambda x: pd.Series(
                x['mass'].values @ x[['vx', 'vy', 'vz']].values /
                x.mass.sum(), index=['vx', 'vy', 'vz']))
    ...
    def green_kubo(self, temp=298.15):
        ...
        for i in range(len(self.ave)):
            self.cond[i] = \
                self.ave[i] / 3 / k / temp / self.volume * el ** 2 / 10 ** 5
    ...
    def save(self):
        ...
        np.savetxt(f'{self.working_dir}/charge_flux.csv', charge_flux.T,
                   delimiter=',', header=col_names, comments='')
        np.savetxt(f'{self.working_dir}/integral.csv', integral.T,
                   delimiter=',', header=col_names, comments='')
        np.savetxt(f'{self.working_dir}/conductivity.csv',
                   self.cond.reshape(1, self.cond.shape[0]), delimiter=',',
                   header=mol_names, comments='')
    ...
    def plot(self):
        ...
        for i in range(len(self.tot_flux)):
            ax.plot(np.array(self.time), self.tot_flux[i], label=i, linewidth=2,
                    color=next(colors))
        ...
        ax.set_ylabel(r'$\mathrm{\langle J(t)\cdot J(0)\rangle dt}$',
                      fontsize=18)
        fig.tight_layout()
        fig.savefig(f'{self.working_dir}/tot_flux.png', bbox_inches='tight',
                    pad_inches=0.1)
        ...
        _set_axis(ax, axis='both')
        for i in range(len(self.integral)):
            ax.plot(np.array(self.time), self.integral[i], label=i, linewidth=2,
                    color=next(colors))
        ax.legend(fontsize=18, frameon=False)
        ax.set_xscale('log')
        ax.xaxis.set_major_formatter(ScalarFormatter())
        ...
        ax.set_ylabel(
            ...
            fontsize=18)
        fig.tight_layout()
        fig.savefig(f'{self.working_dir}/integral.png', bbox_inches='tight',
                    pad_inches=0.1)

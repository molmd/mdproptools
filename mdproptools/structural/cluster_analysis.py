...
from rdf_cn import _calc_rsq, _calc_atom_type, _define_mol_cols
...
def get_clusters(dump_pattern, atom_type, r_cut, full_trajectory=False,
                 frame=None, num_mols=None, num_atoms_per_mol=None,
                 elements=None, alter_atom_ids=False,
    ...
    for index, dump in enumerate(dumps):
        ...
        for i in atoms:
            ...
            fin_df = pd.DataFrame(data_i, columns=['id', 'x', 'y', 'z']). \
                ...
                reset_index()
            frame_number = '{}{}'.format(
                '0'*(len(str(len(dumps)))-len(str(index))),
                index)
            filename = \
                'Cluster_{}_{}{}.xyz'.format(frame_number,
                                             '0'*(len(str(len(atoms))) -
                                                  len(str(counter-1))),
                                             counter-1)

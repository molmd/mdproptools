...
def get_msd_from_dump(
    ...
    if msd_type == "com":
        ...
        msd.columns = ["_".join([str(i) for i in v]) for v in msd.columns.values]
    ...
    if avg_interval:
        ...
        msd_df[disps] = (
            msd_df[coords].groupby(id_cols).transform(lambda x: (x - x.shift(1)) ** 2)
        )
...
def get_diff(msd, initial_time=None, final_time=None, dimension=3, working_dir=None):
    ...
    for col in msd.columns:
        if "msd" in col.lower():
            msd_df = msd[
                (msd["Time (ps)"] >= initial_time.get(counter, min_t))
            ...
            diff.append(
                [
                    model.params[0] / (2 * dimension * (10 ** 8)),
            ...
            with open(
                os.path.join(working_dir, "{}_diff.txt".format(filename)), "w"
            ) as f:
    ...
    diffusion = pd.DataFrame(
        ...
        index=[col.split(" msd")[0] for col in msd.columns if "msd" in col.lower()],
    ).T
...
def _define_mol_cols(dump, num_mols=None, num_atoms_per_mol=None, mass=None):
    ...
    if not mass:
        ...
        df = pd.DataFrame(
            dump.data[["id", "type", "xu", "yu", "zu", "mass"]],
            columns=["id", "type", "xu", "yu", "zu", "mass"],
        )
    else:
        df = pd.DataFrame(
            dump.data[["id", "type", "xu", "yu", "zu"]],

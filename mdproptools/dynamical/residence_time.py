...
class ResidenceTime:
    def __init__(self, r_cut, partial_relations, filename, dt=1,
                 working_dir=None):
        self.r_cut = r_cut
    ...
    def calc_auto_correlation(self):
        ...
        for ind, dump in enumerate(self.dumps):
            ...
            for kl in range(0, len(self.relation_matrix)):
                ...
                k_data = full_df[full_df['type'] == k].drop('type',
                                                            axis=1).values
                l_data = full_df[full_df['type'] == l].drop('type',
                                                            axis=1).values
                h_matrix = []
                for idx, k_row in enumerate(k_data):
                    data_i, rsq = _calc_rsq(k_row, l_data, lx, ly, lz, 0)
                    h = (rsq > self.r_cut[kl][0] ** 2) & (
                            rsq <= self.r_cut[kl][1] ** 2)
                    if k == l:
                        h[idx] = False
                    h_matrix.append(h.reshape(1, len(h)))
                h_matrix = np.concatenate(h_matrix, axis=0)
                h_matrix_dict[atom_pair] = h_matrix_dict.get(atom_pair, []) + \
                                           [h_matrix]
            ...
            print('Finished computing auto-correlation function for timestep',
                  dump.timestep)
        for kl in range(0, len(self.relation_matrix)):
            k, l = self.relation_matrix[kl]
            atom_pair = f'{k}-{l}'
            ...
            for delta_time in range(cl):
                for i in range(cl):
                    s = np.sum(h_matrix_dict[atom_pair][i] *
    ...
    def fit_auto_correlation(self):
        ...
        for col in self.corr_df:
            if col != 'Time (ps)':
                ...
                residence_time[col]['residence_time (ps)'] = \
                    np.sum(np.multiply(popt[0:4], popt[4:]))
    ...
    def plot_results(self):
        for atom_pair in self.atom_pairs:
            try:
                ...
                _set_axis(ax)
                fit = self._exp_func(self.corr_df['Time (ps)'],
                                     *self.res_time_df[atom_pair][0:8].values)
                ax.scatter(self.corr_df['Time (ps)'], self.corr_df[atom_pair],
                           color='red', label='original')
                ax.plot(self.corr_df['Time (ps)'], fit, color='black',
                        label='fit')
                ax.legend(frameon=False, fontsize=20)
                ax.set_xlabel('Time (ps)', fontsize=20)
                ax.set_ylabel('C(t)', fontsize=20)
                fig.savefig(self.working_dir + f'/{atom_pair}_fit.png',
...
class Displacement:
    def __init__(self, atom_types, residence_time, filename, dt=1,
                 save_mode=True, working_dir=None):
    ...
    def calc_dist(self):
        ...
        for ind, dump in enumerate(self.dumps):
            ...
            for atom_type in self.atom_types:
                atoms_coord = \
                    full_df[full_df['type'] == atom_type][['id', 'x', 'y', 'z']]
                atoms_coord['Time (ps)'] = dump.timestep * self.dt
                atoms_data[atom_type] = atoms_data.get(atom_type, []) + \
                                        [atoms_coord]

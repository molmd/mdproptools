...
def _rdf_loop(data, relation_matrix, num_relations, lengths, r_cut, ddr,
    ...
    for i in range(0, data.shape[0] - 1):
        ...
        for j in data_i[:, 5].astype(np.int64):
            rdf_full[j] += 2
        for kl in range(0, num_relations):
...
def _rdf_mol_loop(atom_data, mol_data, relation_matrix, num_relations, lengths,
    ...
    """
    lx, ly, lz = lengths
    for i in range(0, atom_data.shape[0]):
...
def _save_rdf(radii, relation_matrix, path_or_buf, save_mode, rdf_part_sum,
    ...
    if save_mode:
        ...
        print("RDF results are written to pd.DataFrame and csv file")
    else:
        final_df
        print("RDF results are written to pd.DataFrame")
    return final_df
...
def calc_atomic_cn(r_cut, bin_size, num_types, mass, partial_relations, filename,

...
from rdf_cn import _calc_rsq, _calc_atom_type, _initialize, _define_dataframes, \
    _save_rdf
...
def calc_number_density(dump_pattern, surface_atom, atom_types, bin_size,
                        dist_from_interface, axis_norm_interface, num_mols=None,
    ...
    partial_relations = np.array((np.full(shape=len(atom_types),
                                          fill_value=surface_atom,
                                          dtype=np.int),
                                  atom_types))
    dumps, num_bins, radii, num_files, num_relations = \
        _initialize(dist_from_interface,
                    bin_size,
    ...
    for dump in dumps:
        ...
        # provided direction
        min_dist = ref_df[
            ref_df[atom_types_col] == surface_atom][axis_norm_interface].min()
        max_dist = ref_df[
            ref_df[atom_types_col] == surface_atom][axis_norm_interface].max()
        ...
        for i, j in enumerate(atom_types):
            b = ref_df[(ref_df[atom_types_col] == j) &
        ...
        box_lengths_dict = {"x": box_lengths[0],
                            ...
                            "z": box_lengths[2]}
        rho_part = rho_part / (
                np.product([box_lengths_dict[k] for k in box_lengths_dict
                            if k != axis_norm_interface]) * bin_size)
    ...
    final_df = _save_rdf(radii, np.asarray(partial_relations).transpose(),
                         os.path.join(working_dir, results_file), save_mode,
                         rho_part_sum)
    return final_df
